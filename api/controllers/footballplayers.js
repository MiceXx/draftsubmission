...
Player = mongoose.model('Footballplayer', playerSchema);
      ...
      .then(docs => {
          ...
          playerList.forEach(player => {
              if(player.age && player.firstname && player.lastname){
                response.push({
                    id: player.id,
                    name_brief: player.firstname.charAt(0)  + '. ' +  player.lastname,
                    age: player.age,
      ...
      .then(docs => {
        ...
        if (responsePlayer) {
          res.status(200).json({
            ...
            name_brief: responsePlayer.firstname.charAt(0)  + '. ' +  responsePlayer.lastname,
